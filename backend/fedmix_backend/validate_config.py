...
# https://github.com/reinefjord/configparser-validator
#
    ...
    for section in config.sections():
        ...
        for conf_key, conf_val in config.items(section):
            if conf_key not in template[section]:
                errors.append(
                    "Invalid key in section '{section}': '{key}'".format(
                        section=section, key=conf_key))
                continue
            ...
            try:
                ...
                errors.append(("Invalid value for key '{key}' in section "
                               "'{section}': '{val}'").format(section=section,
                                                              key=conf_key,
                                                              val=conf_val))
    return errors